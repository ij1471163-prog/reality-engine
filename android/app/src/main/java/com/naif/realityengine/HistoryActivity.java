...
public class HistoryActivity extends AppCompatActivity {
    ...
    // ── Tag constants — لا تصادم مع card tags ──────────
    private static final String TAG_STATS = "main_tvStats"; // فريد لـ main layout
    ...
    // ✅ thread-safe timestamp formatter — مُنشأ مرة واحدة
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvStats.setTag(TAG_STATS); // ✅ فريد — لا تصادم مع card tags
    ...
    private void applyFilter() {
        List<BackupManager.Session> newList = allSessions.stream()
        ...
        // ✅ DiffUtil — يُعيد رسم العناصر المتغيّرة فقط
        DiffUtil.DiffResult diff = DiffUtil.calculateDiff(new DiffUtil.Callback() {
            @Override public int getOldListSize() { return filteredList.size(); }
            @Override public int getNewListSize() { return newList.size(); }
            @Override public boolean areItemsTheSame(int o, int n) {
                String oid = filteredList.get(o).sessionId;
                String nid = newList.get(n).sessionId;
                return oid != null && oid.equals(nid);
            }
            @Override public boolean areContentsTheSame(int o, int n) {
                BackupManager.Session os = filteredList.get(o);
                BackupManager.Session ns = newList.get(n);
                return Objects.equals(os.fileName, ns.fileName)
                    && Objects.equals(os.createdAt, ns.createdAt)
                    && os.versions.size() == ns.versions.size();
            }
        });
    ...
    private void updateStats() {
        // ✅ استخدام TAG_STATS الفريد بدل "tvStats" الذي يتعارض مع card tags
        View v = getWindow().getDecorView().findViewWithTag(TAG_STATS);
        if (v instanceof TextView) {
            int total    = filteredList.size();
            int versions = filteredList.stream().mapToInt(s -> s.versions.size()).sum();
            ((TextView) v).setText(total + " ملف  ·  " + versions + " نسخة");
        }
    }
        ...
        class VH extends RecyclerView.ViewHolder {
            ...
            void bind(BackupManager.Session session, boolean expanded) {
                ...
                btnExpand.setOnClickListener(v -> {
                    // ✅ getBindingAdapterPosition بدل getAdapterPosition (deprecated)
                    int pos = getBindingAdapterPosition();
                    if (pos == RecyclerView.NO_ID) return;
                    String sid = session.sessionId;
                    if (sid == null) return; // ✅ null check
                    if (expandedIds.contains(sid)) expandedIds.remove(sid);
    ...
    private void shareFile(String filename, String content, String format) throws IOException {
        ...
        if (!dir.exists() && !dir.mkdirs()) {
            throw new IOException("تعذّر إنشاء مجلد التصدير");
        }
        File file = new File(dir, filename);

        // ✅ StandardCharsets.UTF_8 — لا encoding مجهول
    ...
    private String timestamp() {
        // ✅ ThreadLocal — thread-safe، مُنشأ مرة واحدة
