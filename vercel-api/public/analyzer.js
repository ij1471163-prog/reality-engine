...
function analyzeJava(code, fileName) {
    ...
    lines.forEach((line, i) => {
        ...
        const defMatch = line.match(/\b(\w+)\s*=\s*new\s+\w+|(\w[\w<>]*)\s+(\w+)\s*=/);
        if (defMatch) {
            const varName = defMatch[3] ?? defMatch[1];
            if (varName) definedVars.add(varName);
        }

        // فحص null dereference
        const callMatch = t.match(/^(\w+)\.\w+\s*\(/);
        if (callMatch) {
            const obj = callMatch[1];
            if (!SKIP_JAVA_OBJS.has(obj) && !definedVars.has(obj) && obj !== 'this' && obj !== 'super') {
                issues.push({
                    type:'bug', sev:'m', title:'NPE محتمل: ' + obj + ' قد يكون null',
                    line: i+1, ev: t,
                    fix: 'if (' + obj + ' != null) { ' + t + ' }',
                });
            }
    ...
    lines.forEach((line, i) => {
        const t = line.trim();
        if (!t.startsWith('//') && PATTERNS.javaStringEq.test(line)) {
            issues.push({
                type:'bug', sev:'h', title:'قارن String بـ .equals() بدل ==', line:i+1, ev:t,
                fix: t
                    .replace(/([\w.]+)\s*==\s*("[^"]*")/, '$1.equals($2)')
                    .replace(/("[^"]*")\s*==\s*([\w.]+)/, '$2.equals($1)'),
            });
        }
    });
    ...
    // 7. catch فارغ
    lines.forEach((line, i) => {
        if (PATTERNS.javaCatch.test(line)) {
            const next = lines[i + 1]?.trim() ?? '';
            if (next === '}' || next === '// ignore' || next === '')
                issues.push({
                    type:'bug', sev:'m', title:'catch فارغ يخفي الأخطاء', line:i+1, ev:line.trim(),
...
function suggestJavaFix(issue, code) {
    ...
    if (issue.title.includes('دالة فارغة')) {
        ...
        const firstParam = paramStr.split(',')[0]?.trim().split(' ').pop() ?? 'value';

        if (returnType === 'void')    return '// TODO: implement ' + name;
        if (returnType === 'boolean') return (nl.includes('valid') || nl.includes('check'))
            ? `return ${firstParam} != null;` : 'return false;';
        if (returnType === 'String')  return (nl.includes('format') || nl.includes('to'))
            ? `return String.valueOf(${firstParam});` : 'return null; // TODO: implement';
        if (returnType === 'int' || returnType === 'long')
            return (nl.includes('count') || nl.includes('size')) ? 'return 0;' : 'return 0; // TODO: calculate';
        if (returnType === 'double' || returnType === 'float')
            return 'return 0.0; // TODO: calculate';
        if (returnType.startsWith('List') || returnType.startsWith('ArrayList'))
