// ═══════════════════════════════════════════════════════
// repair_advanced.js v2.0 — طبقة الإصلاح المتقدم (محافظة ومُتحقَّقة)
//
// المبدأ: Advanced Repair → Verification → نجح ⇒ apply / فشل ⇒ rollback
// إلى الأصل + AI_REQUIRED. لا يُطبَّق إلا إصلاح deterministic لا يغيّر
// دلالة الكود، وكل حالة غير مؤكدة تُترك وتُسجَّل بسببها.
//
...
var AdvancedRepair = (() => {
  ...
  function fix(code, fileName, options) {
    ...
    for (const fx of FIXERS) {
      ...
      for (const c of cands) {
        ...
        if (!problem) {
          ...
          if (!problem && analyze) {
            if (baseIssues === undefined) baseIssues = analyzeSafe(analyze, beforeCode, fileName);
            if (baseIssues) {
              const afterIssues = analyzeSafe(analyze, afterCode, fileName);
              if (!afterIssues) problem = 'analysis_failed';
              else if (hasRegression(baseIssues, afterIssues)) problem = 'regression';
              else baseIssues = afterIssues;
            }
          }
        }
    ...
    result.fixed   = result.details.length ? (fam === 'py' ? withPyImports(lines, env.imports) : lines.join('\n')) : code;
